...
1. `软件工程实验成果/实验七_SRS初稿与静态建模/SRS初稿.md`
2. `软件工程实验成果/实验八_动态建模与Petri网/动态模型.md`
3. `软件工程实验成果/实验八_动态建模与Petri网/Petri网模型.md`
4. `Qt图形界面/招聘服务.h`
5. `Qt图形界面/登录窗口.h`
6. `Qt图形界面/工作台窗口.h`
...
3. 数据层：由本地文本文件 `招聘数据.txt` 构成，负责持久化保存业务数据。
...
业务层通过 `load()`、`save()` 和 `persist()` 操作本地数据文件。界面层不直接解析或写入 `招聘数据.txt`。
...
1. `主程序.cpp` 创建 `RecruitmentService`。
2. 调用 `loadOrSeed()`。
...
| 数据持久化 | 数据层和部署视图 | `load()`、`save()`、`招聘数据.txt` |
