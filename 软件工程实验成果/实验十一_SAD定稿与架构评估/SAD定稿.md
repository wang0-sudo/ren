...
4. 当前代码：`Qt图形界面/主程序.cpp`、`Qt图形界面/登录窗口.*`、`Qt图形界面/工作台窗口.*`、`Qt图形界面/招聘服务.*`。
...
当前系统已经形成可运行的 Qt 图形化版本，主要代码位于 `Qt图形界面/`：

| 文件 | 责任 |
|---|---|
| `主程序.cpp` | 程序入口，创建服务对象和登录窗口。 |
| `登录窗口.h/.cpp` | 登录、角色选择、个人注册和企业注册。 |
| `工作台窗口.h/.cpp` | 三类角色工作台、表格展示和按钮事件处理。 |
| `招聘服务.h/.cpp` | 领域对象定义、业务规则、查询和数据持久化。 |
| `招聘数据.txt` | 本地数据文件。 |
| `构建招聘图形界面.bat` | Windows 本地构建脚本。 |
| `run_Qt图形界面.bat` | 运行脚本。 |
